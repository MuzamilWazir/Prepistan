...
export default function PublicLandingPage() {
  ...
  const compulsorySubjects = [
    { name: "English Essay & Précis", marks: "200 Marks", desc: "Master thesis statement formulation, structured essay outlines, précis writing & vocabulary building templates.", status: "100% Preloaded", tag: "Compulsory" },
    { name: "General Science & Ability", marks: "100 Marks", desc: "Physical sciences, environmental science, quantitative reasoning, and mental ability past papers solved.", status: "Preloaded Node", tag: "Compulsory" },
    { name: "Current & Pakistan Affairs", marks: "200 Marks", desc: "Foreign policy structures, economic challenges, regional dynamics, and constitutional history timelines.", status: "Daily Live Updates", tag: "Compulsory" },
    { name: "Islamic Studies", marks: "100 Marks", desc: "Core Islamic tenets, modern socio-economic challenges, governance in Islam, and comparative religion guides.", status: "Bilingual Templates", tag: "Compulsory" }
  ];
    ...
    <div className={`min-h-screen transition-colors duration-200 font-sans ${darkMode ? "dark bg-[#0F172A] text-slate-100" : "bg-slate-50 text-slate-900"}`}>
      ...
      <section id="exam-suites" className="py-16 px-4 max-w-7xl mx-auto border-t border-slate-200/80 dark:border-slate-800/80">
        <div className="text-center mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-8 max-w-2xl mx-auto p-1.5 bg-slate-100 dark:bg-slate-900/60 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
            <button onClick={() => setActiveCategoryTab("all")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategoryTab === "all" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-slate-100"}`}>All Preparation Suites</button>
            <button onClick={() => setActiveCategoryTab("civil")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategoryTab === "civil" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-slate-100"}`}>Civil & Public Services</button>
            <button onClick={() => setActiveCategoryTab("recruitment")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategoryTab === "recruitment" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-slate-100"}`}>Recruitment & Agencies</button>
            <button onClick={() => setActiveCategoryTab("admissions")} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeCategoryTab === "admissions" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-slate-100"}`}>Entry Admissions</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {examSuites
            ...
            .map((suite, idx) => (
              <div key={idx} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-6 flex flex-col justify-between hover:border-emerald-500/40 hover:shadow-lg hover:shadow-emerald-500/[0.02] transition-all group relative overflow-hidden">
                ...
                <div>
                  <div className="flex items-start justify-between gap-2 mb-4">
                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-100">{suite.icon}</div>
                    <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[9px] font-mono font-bold uppercase rounded">{suite.badge}</span>
                  </div>
                  <span className="text-[10px] font-mono uppercase tracking-wider font-bold text-slate-400 dark:text-slate-500 block mb-1">{suite.tag}</span>
                  <h3 className="text-base font-black text-slate-950 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{suite.name}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">{suite.desc}</p>
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {suite.tests.map((test, tIdx) => (
                        <span key={tIdx} className="px-2 py-0.5 bg-slate-50 dark:bg-slate-800/60 hover:bg-slate-100 text-slate-600 dark:text-slate-300 text-[10px] font-semibold rounded border border-slate-150 dark:border-slate-800">{test}</span>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      <section id="interactive-quiz" className="py-16 bg-slate-100/50 dark:bg-slate-900/30 border-y border-slate-200/80 dark:border-slate-800/80 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 max-w-lg mx-auto">Test your general knowledge of Pakistan constitutional affairs right now. Click any option to see immediate feedback.</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="px-2 py-0.5 bg-purple-150 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400 text-[10px] font-mono font-bold uppercase rounded">Constitutional History</span>
              <span className="text-xs text-slate-400 dark:text-slate-500">• CSS/PMS Compulsory Subject</span>
            </div>

            <p className="text-xs sm:text-sm font-bold text-slate-800 dark:text-white mb-6 leading-relaxed">{sampleQuestion.question}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              {sampleQuestion.options.map((opt, idx) => {
                ...
                if (isAnswered) {
                  if (opt.isCorrect) btnStyle = "bg-emerald-500/10 dark:bg-emerald-500/15 border-emerald-500 text-emerald-600 dark:text-emerald-400 font-bold";
                  else if (selectedOption === idx) btnStyle = "bg-rose-500/10 dark:bg-rose-500/15 border-rose-500 text-rose-600 dark:text-rose-400";
                  else btnStyle = "opacity-50 border-slate-200 dark:border-slate-800";
                }
                return (
                  <button key={idx} onClick={() => handleOptionClick(idx)} className={`w-full p-3 text-left text-xs rounded-xl border transition-all ${btnStyle} flex items-center justify-between`} disabled={isAnswered}>
                    <span>{opt.text}</span>
                    {isAnswered && opt.isCorrect && <span className="text-[9px] font-bold text-emerald-500 font-mono uppercase bg-emerald-500/10 px-1.5 py-0.5 rounded">Correct</span>}
                  </button>
                );
              })}
            </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800/80 animate-in fade-in duration-300">
                <div className="flex gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-normal">{sampleQuestion.explanation}</p>
                  </div>
                </div>
      ...
      <section id="features" className="py-20 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-3 max-w-xl mx-auto leading-relaxed">Every feature is calibrated to Pakistan Civil Service exam syllabus guides. We replace bulky physical study sheets with dynamic modules.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-purple-500/10 dark:bg-purple-500/15 text-purple-600 dark:text-purple-400 flex items-center justify-center mb-4 font-bold"><Sparkles className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2 flex items-center gap-1.5">Simulated AI Tutor Gating<span className="text-[8px] font-mono font-bold uppercase tracking-wider text-purple-600 bg-purple-500/10 px-1.5 py-0.5 rounded">Smart</span></h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Need feedback on a Foreign Policy outline or constitutional analysis? Consult the 24/7 AI tutor, generate outlines, or check preloaded essay topics.</p>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mb-4 font-bold"><BookOpen className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2">Syllabus &amp; Course Log Tracker</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Track your preparation percent for compulsory and optional subjects. Mark sections as completed, save bookmark sheets, and view recommended reading materials.</p>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center mb-4 font-bold"><Trophy className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2">Daily MPT mock challenges</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Fast-loading current affairs, science, and history MCQ modules with immediate ranking. Build daily study streaks, score coins, and climb the public leaderboard.</p>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-blue-500/10 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 flex items-center justify-center mb-4 font-bold"><Newspaper className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2">Officer&apos;s Corner Columns</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Read verified, original study logs, CSS analysis dossiers, and foreign policy summaries published directly by competitive mentors and qualified CSP guides.</p>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-rose-500/10 dark:bg-rose-500/15 text-rose-600 dark:text-rose-400 flex items-center justify-center mb-4 font-bold"><Shield className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2">Encrypted Saved Vault</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Bookmark crucial, complex MCQs or notes with offline client-side support. Retrieve saved topics instantly during last-minute exam revisions.</p>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-teal-500/10 dark:bg-teal-500/15 text-teal-600 dark:text-teal-400 flex items-center justify-center mb-4 font-bold"><Zap className="w-5 h-5" /></div>
            <h3 className="text-sm font-black text-slate-950 dark:text-white mb-2">Premium Diagnostic Mocks</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal">Unlock simulated full-length exam modes with timer restrictions. Access standard evaluations and review score curves.</p>
          </div>
        </div>
      </section>
      ...
      <section id="compulsory" className="py-20 bg-slate-100/50 dark:bg-slate-900/35 border-y border-slate-200/80 dark:border-slate-800/80 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-3 max-w-2xl mx-auto leading-relaxed">We preload comprehensive syllabi and study progress matrices. Whether you are tracking CSS compulsory units, provincial general knowledge nodes, NTS quantitative criteria, or entry exam modules—you can start logging progress instantly.</p>
          </div>
              ...
              <div key={idx} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 flex flex-col justify-between hover:border-emerald-500/40 transition-all group">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] font-mono font-bold uppercase rounded">{sub.tag}</span>
                    <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-mono">{sub.marks}</span>
                  </div>
                  <h3 className="text-sm font-black text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{sub.name}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">{sub.desc}</p>
                </div>
      ...
      <section id="about" className="py-20 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-3 max-w-xl mx-auto">Real stories from CSS, PMS, MDCAT, and FIA/NTS candidates who leveraged gamified study trackers to secure stellar results.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 relative shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-1 mb-4 text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 fill-current" />)}</div>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 italic leading-relaxed mb-6">&quot;The GSA notes tracker on Prepistan kept my revision schedules extremely tidy. I went from struggling with general science syllabus depth to securing a stellar 76 score. The simulated current affairs modules are incredibly high quality.&quot;</p>
            </div>
            <div className="flex items-center gap-3 mt-4">
              <div className="w-9 h-9 rounded-full bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-bold text-xs">ST</div>
              <div><span className="text-xs font-bold text-slate-900 dark:text-white block">Sana Tariq</span><span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Allocated Assistant Commissioner (48th STP)</span></div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 relative shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-1 mb-4 text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 fill-current" />)}</div>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 italic leading-relaxed mb-6">&quot;The leaderboard and streak mechanisms kept me studying every single night after my day job. I answered over 1,500 questions in Islamiyat and current affairs. Secure local persistence is outstandingly executed.&quot;</p>
            </div>
            <div className="flex items-center gap-3 mt-4">
              <div className="w-9 h-9 rounded-full bg-purple-100 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400 flex items-center justify-center font-bold text-xs">BA</div>
              <div><span className="text-xs font-bold text-slate-900 dark:text-white block">Bilal Ahmed</span><span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">CSS Aspirant 2026, Lahore</span></div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 relative shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-1 mb-4 text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 fill-current" />)}</div>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 italic leading-relaxed mb-6">&quot;Prepistan&apos;s high-yield MDCAT Biology and Chemistry vaults helped me refine my concepts. The custom simulated feedback and timed sandbox tests are a game changer for university entries.&quot;</p>
            </div>
            <div className="flex items-center gap-3 mt-4">
              <div className="w-9 h-9 rounded-full bg-pink-100 dark:bg-pink-950/40 text-pink-700 dark:text-pink-400 flex items-center justify-center font-bold text-xs">ZM</div>
              <div><span className="text-xs font-bold text-slate-900 dark:text-white block">Zainab Malik</span><span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">MDCAT Candidate (scored 188/200)</span></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-8 bg-slate-50 dark:bg-[#0A0F1D] border-t border-slate-200 dark:border-slate-900 px-4 text-center">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
          ...
          <div><span>&copy; {new Date().getFullYear()} Prepistan. Built to aid competitive education across Pakistan.</span></div>
        </div>
      </footer>
